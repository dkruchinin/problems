...
static std::string find_longest_palindrome(const std::string &str)
{
    if (str.size() <= 1) {
        return str;
    }
    ...
    for (int i = 0; i < str.size(); i++) {
        ...
        if (l * 2 > max_len_so_far) {
            max_len_so_far = l * 2;
            idx = i - l;
        }
    }

    return test_str.substr(idx, max_len_so_far);
}
...
int main(int argc, char *argv[])
{
    std::string in;
    std::cin >> in;
    std::cout << find_longest_palindrome(in) << std::endl;
